...
add_shortcode('tabs_import_cron_meks_flexible_shortcodes', 'tabs_import_cron_meks_flexible_shortcodes');
add_action('tabs_import_cron_meks_flexible_shortcodes', 'tabs_import_cron_meks_flexible_shortcodes');


function tabs_import_cron_meks_flexible_shortcodes(){
    ...
    if ( $wp_query->have_posts() ) :
        while ( $wp_query->have_posts() ) : $wp_query->the_post();
            ...
            if( strpos($post_content, '[mks_accordion') !== false){
                $tabs = tabs_str_between_all($post_content, "[mks_accordion", "[/mks_accordion]");

                if(!empty($tabs))
                foreach ($tabs as $tab_content){

                    $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='mks_accordion_item');
            ...
            if( strpos($post_content, '[mks_tabs') !== false){
                $tabs = tabs_str_between_all($post_content, "[mks_tabs", "[/mks_tabs]");

                if(!empty($tabs))
                    foreach ($tabs as $tab_content){

                        $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='mks_tab_item');
        ...
        wp_clear_scheduled_hook('tabs_import_cron_meks_flexible_shortcodes');
