...
add_shortcode('tabs_import_cron_shortcodes_ultimate', 'tabs_import_cron_shortcodes_ultimate');
add_action('tabs_import_cron_shortcodes_ultimate', 'tabs_import_cron_shortcodes_ultimate');


function tabs_import_cron_shortcodes_ultimate(){
    ...
    if ( $wp_query->have_posts() ) :
        while ( $wp_query->have_posts() ) : $wp_query->the_post();
            ...
            if( strpos($post_content, '[su_accordion') !== false){
                $tabs = tabs_str_between_all($post_content, "[su_accordion", "[/su_accordion]");

                if(!empty($tabs))
                foreach ($tabs as $tab_content){

                    $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='su_spoiler');
            ...
            if( strpos($post_content, '[su_tabs') !== false){
                $tabs = tabs_str_between_all($post_content, "[su_tabs", "[/su_tabs]");

                if(!empty($tabs))
                    foreach ($tabs as $tab_content){

                        $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='su_tab');
        ...
        wp_clear_scheduled_hook('tabs_import_cron_shortcodes_ultimate');
