...
add_shortcode('tabs_import_cron_wc_shortcodes', 'tabs_import_cron_wc_shortcodes');
add_action('tabs_import_cron_wc_shortcodes', 'tabs_import_cron_wc_shortcodes');


function tabs_import_cron_wc_shortcodes(){
    ...
    if ( $wp_query->have_posts() ) :
        while ( $wp_query->have_posts() ) : $wp_query->the_post();
            ...
            if( strpos($post_content, '[wc_accordion') !== false){
                $tabs = tabs_str_between_all($post_content, "[wc_accordion", "[/wc_accordion]");

                if(!empty($tabs))
                foreach ($tabs as $tab_content){

                    $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='wc_accordion_section');
            ...
            if( strpos($post_content, '[wc_tabgroup') !== false){
                $tabs = tabs_str_between_all($post_content, "[wc_tabgroup", "[/wc_tabgroup]");

                if(!empty($tabs))
                    foreach ($tabs as $tab_content){

                        $shortcode_content = tabs_nested_shortcode_content($tab_content, $child_tag='wc_tab');
